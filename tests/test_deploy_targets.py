...
def test_repository_deploy_targets_load() -> None:
    ...
    assert (
        "scripts/kubernetes_dry_run.py --target k3s --require-cluster --require-amd-gpu "
...
def test_repository_k3s_declares_real_proof_artifact_bundle() -> None:
    ...
    assert (
        "scripts/kubernetes_dry_run.py --verify-artifact-dir local-kubernetes-proof/k3s"
...
def test_validate_deploy_targets_rejects_kubernetes_proof_without_artifact_dir(
    ...
    k3s = _kubernetes_target_with_verification(
        commands=("scripts/kubernetes_dry_run.py --target k3s --require-cluster --require-amd-gpu",)
    )
...
def test_deploy_target_check_script_runs() -> None:
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts" / "deploy_target_check.py")],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_deploy_target_check_script_json_output() -> None:
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts" / "deploy_target_check.py"), "--json"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_pre_commit_runs_deploy_target_check() -> None:
    ...
    assert hook["entry"] == ".venv/bin/python scripts/deploy_target_check.py"
...
def test_ci_runs_deploy_target_contract_gate() -> None:
    ...
    assert "scripts/deploy_target_check.py" in workflow

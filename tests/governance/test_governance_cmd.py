...
def test_governance_cli_text_output(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "governance OK: 10 checks (status=ok, warnings=0, infos=0, errors=0)" in out
...
def test_governance_cli_json_output(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert summary["status_counts"] == {"failed": 0, "warning": 0, "info": 0, "ok": 10}
    assert summary["payload_count"] == 10
...
def test_governance_check_script_json_includes_structured_gate_payloads() -> None:
    ...
    assert payload["summary"]["payload_count"] == 10
    ...
    assert components["payload"]["service_count"] == 47  # +agent-{pydanticai,agno,db,ui} 2026-06-09
...
def test_governance_check_script_runs() -> None:
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    assert "governance OK: 10 checks (status=ok, warnings=0, infos=0, errors=0)" in result.stdout

"""Tests для scripts/checks/audit_forbidden.py — поведение RULES + opt-out logic.
...
_AUDIT_SCRIPT = _REPO_ROOT / "scripts" / "checks" / "audit_forbidden.py"
...
def test_audit_detects_mutable_compose_image_tag(tmp_path: Path) -> None:
    f = tmp_path / "compose.yml"
    ...
    assert code == 1
    assert "mutable_image_tag" in out
    assert "redis:latest" in out


def test_audit_detects_mutable_dockerfile_base(tmp_path: Path) -> None:
    f = tmp_path / "Dockerfile"
    f.write_text("FROM postgres:latest\n", encoding="utf-8")

    code, out = _run_audit(tmp_path, fail=True)

    assert code == 1
    assert "mutable_image_tag" in out
    assert "postgres:latest" in out


def test_audit_allows_latest_policy_comment(tmp_path: Path) -> None:
    f = tmp_path / "models.yaml"
    f.write_text("# НИКАКОГО :latest — pin concrete model filename\n", encoding="utf-8")
...
def test_audit_ansible_galaxy_dir_excluded(tmp_path: Path) -> None:
    galaxy = tmp_path / ".galaxy"
    galaxy.mkdir()
    (galaxy / "vendor.yml").write_text("arch: arm64\n")
    code, _ = _run_audit(tmp_path, fail=True)
    assert code == 0


def test_audit_detects_tracked_agent_local_artifacts(tmp_path: Path) -> None:
    """Agent-local notes may exist on disk, but must never be tracked by Git."""
    subprocess.run(["git", "init"], cwd=tmp_path, check=True, capture_output=True, text=True)
    planning = tmp_path / ".planning"
    planning.mkdir()
    (planning / "STATE.md").write_text("# local state\n", encoding="utf-8")
    docs_plans = tmp_path / "docs" / "superpowers" / "plans"
    docs_plans.mkdir(parents=True)
    (docs_plans / "work.md").write_text("# local plan\n", encoding="utf-8")
    (tmp_path / "CLAUDE.md").write_text("# local agent notes\n", encoding="utf-8")
    subprocess.run(
        [
            "git",
            "add",
    ...
    assert code == 1
    assert "agent_artifact_tracked" in out
    assert ".planning/STATE.md" in out
    assert "docs/superpowers/plans/work.md" in out

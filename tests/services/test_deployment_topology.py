...
def test_validate_topology_profiles_reports_standard_lanes_clean() -> None:
    # H.4: default now validates all 14 isolation lanes via ALL_PROFILE_SETS
    from agmind.services.profile_sets import ALL_PROFILE_SETS

    report = validate_topology_profiles()

    assert report.ok is True
    assert len(report.profiles) == 14  # +agents-pydantic +agents-agno 2026-06-08
    ...
    assert "topology OK: 14 profile sets" in text

...
def test_render_kubernetes_includes_namespace_deployment_and_service() -> None:
    ...
    assert container["image"] == f"qdrant/qdrant:v1.18.0@sha256:{VALID_SHA256}"
...
def test_render_to_string_rejects_unknown_excluded_service_names() -> None:
    with pytest.raises(ValueError, match="Unknown excluded services: missing-service"):
        render_to_string(
            profiles=["core"],
            exclude_services=["traefik", "missing-service"],
            namespace="agmind",
        )
...
def test_render_to_string_rejects_unknown_profile_names() -> None:
    with pytest.raises(ValueError, match="Unknown profiles requested: missing-profile"):
        render_to_string(profiles=["core", "missing-profile"], namespace="agmind")
...
def test_render_to_string_rejects_unknown_explicit_service_names() -> None:
    with pytest.raises(ValueError, match="Unknown services requested: missing-service"):
        render_to_string(services=["traefik", "missing-service"], namespace="agmind")
...
def test_render_to_string_rejects_explicit_services_with_missing_dependencies() -> None:
    with pytest.raises(ValueError, match="Missing dependencies for selected services"):
        render_to_string(services=["dify-api"], namespace="agmind")


def test_render_to_string_rejects_explicit_services_when_all_are_omitted() -> None:
    with pytest.raises(ValueError, match="No Kubernetes-renderable services selected"):
        render_to_string(services=["dozzle"], namespace="agmind")
...
def test_render_to_string_explicit_services_ignore_unused_unknown_profiles() -> None:
    ...
    assert _object_names(docs, kind="Deployment") == {"traefik"}

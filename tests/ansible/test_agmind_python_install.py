...
def test_agmind_package_install_is_not_editable_from_operator_home() -> None:
    ...
    install_task = next(
        task for task in tasks if str(task.get("name", "")).startswith("Install agmind package")
    )

    pip_args = install_task["ansible.builtin.pip"]
    # Runtime-only on deployed nodes — no [dev] toolchain (audit M#27).
    assert pip_args["name"] == ["agmind @ file://{{ playbook_dir }}/.."]

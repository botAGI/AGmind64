...
def test_governance_cli_text_output(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "governance OK: 7 checks (status=warning, warnings=4, infos=1, errors=0)" in out
...
def test_governance_cli_json_output(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["summary"] == {
        ...
        "component_contracts": 9,
        "service_descriptors": 34,
        "deploy_targets": 3,
        "tool_candidates": 11,
        "constraint_planes": 5,
        ...
        "total_errors": 0,
        "failed_checks": [],
        "warning_checks": ["kubernetes-render"],
        "info_checks": ["topology"],
        "check_health": [
            {
                "name": "components",
                "ok": True,
    ...
    assert components["payload"]["contract_count"] == 9
...
def test_governance_check_script_json_includes_structured_gate_payloads() -> None:
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts" / "governance_check.py"), "--json"],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    assert payload["summary"]["payload_count"] == 7
    assert payload["summary"]["total_warnings"] == 4
    assert payload["summary"]["total_infos"] == 1
    assert payload["summary"]["total_errors"] == 0
    assert payload["summary"]["health_status"] == "warning"
    ...
    assert payload["summary"]["warning_checks"] == ["kubernetes-render"]
    assert payload["summary"]["info_checks"] == ["topology"]
    assert payload["summary"]["check_health"][4] == {
        "name": "topology",
        "ok": True,
        "status": "info",
        "warnings": 0,
    ...
    assert components["payload"]["service_count"] == 34
    assert deploy_targets["payload"]["target_count"] == 3
    assert tool_candidates["payload"]["candidate_count"] == 11
...
def test_governance_check_script_runs() -> None:
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    assert (
        "governance OK: 7 checks (status=warning, warnings=4, infos=1, errors=0)" in result.stdout
    )
...
def test_pre_commit_runs_governance_check_for_aggregate_files() -> None:
    ...
    assert hook["entry"] == ".venv/bin/python scripts/governance_check.py"
    assert "agmind/governance\\.py" in hook["files"]
    assert "agmind/cli/governance_cmd\\.py" in hook["files"]
    assert "scripts/governance_check\\.py" in hook["files"]
    assert "scripts/topology_check\\.py" in hook["files"]
    assert "scripts/kubernetes_render_check\\.py" in hook["files"]
    assert "scripts/kubernetes_proof_workflow_check\\.py" in hook["files"]
...
def test_ci_runs_governance_summary_gate() -> None:
    ...
    assert "scripts/topology_check.py" in workflow
    assert "scripts/governance_check.py" in workflow
...
def test_kubernetes_proof_workflow_check_script_json_output() -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "kubernetes_proof_workflow_check.py"),
            "--json",
        ],
        capture_output=True,
        text=True,

...
from agmind.core.secrets import (
    _MASK,
    _is_valid_key,
    generate_secret,
    get_creds_path,
...
def test_write_creds_removes_temp_file_and_preserves_existing_on_chmod_failure(
    ...
    def fail_tmp_chmod(path: str | bytes | Path, mode: int, *args: object) -> None:
        candidate = Path(path)
        if "creds" in candidate.name and candidate != p:
            raise PermissionError("chmod denied")
        if isinstance(path, Path):
            original_path_chmod(path, mode)
        else:
            original_os_chmod(path, mode, *args)

    monkeypatch.setattr(Path, "chmod", fail_tmp_chmod)
    monkeypatch.setattr("agmind.core.secrets.os.chmod", fail_tmp_chmod)

    with pytest.raises(PermissionError, match="chmod denied"):
        write_creds({"K2": "new-secret"}, path=p)

    assert p.read_text(encoding="utf-8") == original
    assert not any(item.name != "creds.txt" for item in tmp_path.glob("*creds*"))

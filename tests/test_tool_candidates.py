...
def test_tool_candidate_check_script_runs() -> None:
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts" / "tool_candidate_check.py")],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_tool_candidate_check_script_json_output() -> None:
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts" / "tool_candidate_check.py"), "--json"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_pre_commit_runs_tool_candidate_check() -> None:
    ...
    assert hook["entry"] == ".venv/bin/python scripts/tool_candidate_check.py"
...
def test_ci_runs_tool_candidate_contract_gate() -> None:
    ...
    assert "scripts/tool_candidate_check.py" in workflow
...
def test_accepted_candidate_requires_matching_service_descriptor() -> None:
    import scripts.tool_candidate_check as checker

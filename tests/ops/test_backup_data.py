...
def test_mongo_dump_command_password_via_env_not_argv() -> None:
    from agmind.ops.backup_data import DbDumpSource

    src = DbDumpSource(
        "dbdump/komodo-mongo", "agmind-komodo-mongo", "mongo", "komodo", "", "s3cret"
    )
    cmd = src.dump_command()
    assert "mongodump" in " ".join(cmd) and "--archive" in " ".join(cmd)
    assert "MONGO_PW=s3cret" in cmd  # password via -e env
    assert "s3cret" not in " ".join(c for c in cmd if not c.startswith("MONGO_PW="))  # not in argv


def test_komodo_mongo_becomes_logical_dump_not_volume_tar() -> None:
    """live-audit MED hot-volume-tar-not-quiesced: komodo-mongo is a logical mongodump, not a
    crash-consistent tar of the live WiredTiger dir."""
    descriptors = {
        "komodo-mongo": _svc(
            "komodo-mongo",
            volumes=["/var/lib/agmind/komodo-mongo/data:/data/db"],
            env={"MONGO_INITDB_ROOT_USERNAME": "${KOMODO_DATABASE_USERNAME:-komodo}"},
        ),
    }
    sources = data_sources(
        ["komodo-mongo"],
        descriptors,
        env={"KOMODO_DATABASE_PASSWORD": "pw", "KOMODO_DATABASE_USERNAME": "komodo"},
    )
    dumps = [s for s in sources if isinstance(s, DbDumpSource)]
    vols = [s for s in sources if isinstance(s, DataVolumeSource)]
    assert len(dumps) == 1 and not vols  # logical dump, NOT a volume tar
    assert dumps[0].engine == "mongo" and dumps[0].user == "komodo" and dumps[0].password == "pw"


def test_mongo_restore_archive_drop_reads_stdin() -> None:
    from agmind.ops.backup_data import DbDumpSource, restore_db_command

    src = DbDumpSource("dbdump/komodo-mongo", "agmind-komodo-mongo", "mongo", "komodo", "", "pw")
    cmd = restore_db_command(src)
    assert cmd[:3] == ["docker", "exec", "-i"]  # stdin
    joined = " ".join(cmd)
    assert "mongorestore" in joined and "--archive" in joined and "--drop" in joined

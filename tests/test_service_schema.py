...
def test_image_with_digest_accepted() -> None:
    d = _minimal_descriptor(image="qdrant/qdrant@sha256:abc123")
    assert d.image.endswith("abc123")
...
@pytest.mark.parametrize(
    "bad_port",
    ["8080", "8080:abc", "127.0.0.1:abc:8080", ":8080:8080", "127:8080:8080"],
)
...
def test_fq_image_with_digest() -> None:
    d = _minimal_descriptor(digest="abc123def456")
    assert d.fq_image() == "qdrant/qdrant:v1.18.0@sha256:abc123def456"


def test_fq_image_with_prefixed_digest() -> None:
    d = _minimal_descriptor(digest="sha256:abc123def456")
    assert d.fq_image() == "qdrant/qdrant:v1.18.0@sha256:abc123def456"
...
def test_to_legacy_service_basic() -> None:
    ...
    assert legacy.digest == "abc123"  # без `sha256:` префикса
...
def test_to_legacy_service_strips_sha256_prefix() -> None:
    d = _minimal_descriptor(digest="sha256:deadbeef")
    legacy = d.to_legacy_service()
    assert legacy.digest == "deadbeef"

...
Usage:
  python3 scripts/checks/version_check.py                # stdout markdown
  python3 scripts/checks/version_check.py --output report.md
  python3 scripts/checks/version_check.py --json out.json # machine-readable
...
def render_markdown(reports: list[PinReport]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            f"_Generated by `scripts/checks/version_check.py` at {now}._",
        ]
    )
    return "\n".join(lines) + "\n"

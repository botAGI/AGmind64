...
Запуск:
    python scripts/checks/audit_forbidden.py              # отчёт по всему репо
    python scripts/checks/audit_forbidden.py path/to/dir  # по подкаталогу
    python scripts/checks/audit_forbidden.py --fail       # exit 1 при находках (для CI)
    python scripts/checks/audit_forbidden.py --json out.json
...
EXCLUDED_PATHS = {
    ...
    "README.md",
    "CLAUDE.md",
    # Публикационный черновик (field report по Strix Halo-узлу): упоминает старый
    # стек (GB10/CUDA/aarch64) как исторический контекст. Gitignored (habr-article*.md).
    "habr-article-strix-halo-node.md",
    "docs/HARDWARE.md",
    "docs/BENCHMARKS.md",
    "docs/QUICKSTART.md",
    ...
    "scripts/checks/audit_forbidden.py",
    # governance/test_audit_script.py намеренно содержит запрещённые паттерны как
    # fixtures для positive-detection тестов. По семантике = полный
    # `# audit: allow` на файл.
    "tests/governance/test_audit_script.py",
}
...
RULES: list[tuple[str, str, re.Pattern[str]]] = [
    ...
    (
        "mutable_image_tag",
        "Deploy-facing container image uses mutable :latest tag",
        re.compile(
            r"(^\s*image:\s*['\"]?[^'\"\s#]+:latest(?:@|['\"\s#]|$)|"  # audit: allow rule-self-reference
            r"^\s*FROM\s+[^'\"\s#]+:latest(?:@|['\"\s#]|$)|"  # audit: allow rule-self-reference
            r"\b(?:docker|podman)\s+(?:run|pull)\s+[^#\n]*:latest(?:@|['\"\s#]|$))",  # audit: allow rule-self-reference
            re.IGNORECASE,
        ),
    ),
]
...
AGENT_ARTIFACT_RULE = (
    "agent_artifact_tracked",
    "Agent-local planning/context artifacts tracked in Git",
)
...
def scan_tracked_agent_artifacts(root: Path, report: Report) -> None:
    """Detect ignored/local agent artifacts that were force-added to Git."""
    top = _git_root(root)
    if top is None:
        return
    try:
    ...
    try:
        result = subprocess.run(
            ["git", "-C", str(top), "ls-files", "-z", "--", pathspec],
            check=False,
            capture_output=True,
            text=False,
        )
    ...
    for raw in result.stdout.split(b"\0"):
        ...
        if not _is_agent_local_path(path):
            continue
        report.findings.append(
            Finding(
                rule=AGENT_ARTIFACT_RULE[0],
                description=AGENT_ARTIFACT_RULE[1],
                file=path,
                line=1,
                snippet="keep this file local/ignored; do not track agent planning artifacts",
            )
        )
...
def main() -> int:
    ...
    report = Report()
    scan_tracked_agent_artifacts(root, report)
